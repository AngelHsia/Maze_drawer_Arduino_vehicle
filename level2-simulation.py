...
while(True):
    ...
    road_can_go = list()
    for line in lines:
        if line.pos==car.pos or (line.pos+line.axis)==car.pos:
            road_can_go.append(line) # 把可以走的路都放進list road_can_go裡面

    relative_direction = -1
    for road in road_can_go:
        if road.pos==car.pos:
            if cross(car_direction , norm(road.axis))==vec(0,0,-1) :
                relative_direction = 1 # turn right
        else:
            if cross(car_direction , -1*norm(road.axis))==vec(0,0,-1) :
                relative_direction = 1 # turn right
    if relative_direction==-1:
        for road in road_can_go:
            if road.pos==car.pos:
                if dot(car_direction , norm(road.axis))==1:
                    relative_direction = 0 # go straight
            else:
                if dot(car_direction , -1*norm(road.axis))==1:
                    relative_direction = 0 # go straight
    if relative_direction==-1:
        for road in road_can_go:
            if road.pos==car.pos:
                if cross(car_direction , norm(road.axis))==vec(0,0,1) :
                    relative_direction = 3 # turn left
            else:
                if cross(car_direction , -1*norm(road.axis))==vec(0,0,1) :
                    relative_direction = 3 # turn left
    if relative_direction==-1:
        for road in road_can_go:
            if road.pos==car.pos:
                if dot(car_direction , norm(road.axis))==-1:
                    relative_direction = 2 # turn back
            else:
                if dot(car_direction , -1*norm(road.axis))==-1:
                    relative_direction = 2 # turn back

    # Use relative_direction to calculate direction
    direction = (direction + relative_direction) % 4
